...
class _MindMapScreenState extends State<MindMapScreen> {
  ...
  void _fitToScreen() {
    // Center and scale the fixed-size canvas (2000x2000) into the current viewport
    final contextSize = MediaQuery.of(context).size;
    final canvasSize = const Size(2000, 2000);
    final scaleX = contextSize.width / canvasSize.width;
    final scaleY = contextSize.height / canvasSize.height;
    final scale = (scaleX < scaleY ? scaleX : scaleY) * 0.9; // leave margin

    final dx = (contextSize.width - canvasSize.width * scale) / 2.0;
    final dy = (contextSize.height - canvasSize.height * scale) / 2.0;
  ...
  void _zoomIn() {
    ...
    final offset = _transformationController.value.getTranslation();
  ...
  void _zoomOut() {
    ...
    _transformationController.value = Matrix4.identity()
      ..translate(offset.x, offset.y)
      ..scale((scale / 1.2).clamp(0.1, 5.0));
  }

  @override
  void initState() {
    super.initState();
    // Auto-fit after first layout so the map is visible immediately.
    // Run multiple attempts (short delays) because GraphView may compute layout asynchronously.
  ...
  @override
  void dispose() {
    // Dispose the transformation controller to avoid lingering listeners
    try {
  ...
  Widget build(BuildContext context) {
    ...
    for (int idx = 0; idx < widget.nodes.length; idx++) {
      ...
      final nodeWidget = Container(
        constraints: const BoxConstraints(minWidth: 120, minHeight: 70),
        decoration: BoxDecoration(
          color: isRoot ? Colors.amber : const Color(0xFF3A3A3A),
          borderRadius: BorderRadius.circular(10),
          border: Border.all(
            color: isRoot ? Colors.amberAccent : Colors.white24,
            width: isRoot ? 2.0 : 1.0,
          ),
          boxShadow: isRoot
              ? [BoxShadow(color: Colors.amber.withAlpha(40), blurRadius: 8, offset: const Offset(0, 2))]
              : null,
        ),
        child: Padding(
    ...
    // Debug overlay data removed; map will auto-fit and show nodes.

    // Build the GraphView inside try/catch so we surface errors instead of a blank screen
    Widget graphArea;
    try {
      graphArea = Scaffold(
          appBar: AppBar(
            ...
            actions: [
              ...
              IconButton(
                icon: const Icon(Icons.fit_screen, color: Colors.amber),
      ...
      debugPrint('Graph render error: $e\n$st');
      graphArea = Scaffold(
        appBar: AppBar(title: Text(widget.title)),
        ...
        body: SafeArea(
          child: Padding(
            padding: const EdgeInsets.all(16.0),
            child: SingleChildScrollView(
              child: Text('Graph render error:\n\n$e', style: const TextStyle(color: Colors.yellow)),
            ),
          ),
        ),
      );
